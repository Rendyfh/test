...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $juals = Jual::whereRaw("(kurir_id=0 OR kurir_id=?) AND konsumen_rate=0
        AND waktu_pesan>?",
        ...
        $rating_5 = Jual::whereRaw("status_jual='TIBA' AND kurir_id=?",
        [auth()->user()->id])->orderBy('waktu_pesan', 'desc')->take(5)
        ->avg('kurir_rate');
        $rating_semua = Jual::whereRaw("status_jual='TIBA' AND kurir_id=?",
        [auth()->user()->id])->avg('kurir_rate');
        $order_minggu_terakhir = Jual::whereRaw("kurir_id=? AND
        status_jual='TIBA' AND waktu_pesan>=? AND waktu_pesan<?",
        [auth()->user()->id, Carbon::today()->subDays(6),
        Carbon::today()->addDays(1)])->count();
        $order_bulan_ini = Jual::whereRaw("kurir_id=? AND status_jual='TIBA'
        AND waktu_pesan>=? AND waktu_pesan<?",
        [auth()->user()->id, Carbon::today()->firstOfMonth(),
        Carbon::today()->firstOfMonth()->addMonths(1)])->count();
