...
class JualController extends Controller
{
    public function order()
    {
        ...
        $is_cart = Jual::whereRaw("konsumen_id=? AND status_jual='CART'",
    ...
    public function addtocart($id)
    {
        ...
        $jual = Jual::whereRaw("konsumen_id=? AND status_jual='CART'",
        [auth()->user()->id])->first();
        if($jual==null){
        $jual = new Jual();
        ...
        $jual_detail = JualDetail::whereRaw("jual_id=? AND pizza_id=?",
        [$jual->id, $pizza->id])->first();
        if($jual_detail == null){
    ...
    public function postcart(Request $request)
    {
        ...
        {
        $alamat_kirims = AlamatKirim::whereRaw("konsumen_id=? AND 1=1",
        [auth()->user()->id])->orderByRaw('is_default DESC')->get();
        $jual = Jual::whereRaw("konsumen_id=? AND status_jual='CART'",
        [auth()->user()->id])->first();
        $jual_details = JualDetail::whereRaw("jual_id=?",[$jual->id])->get();
        $jual->total_harga = JualDetail::whereRaw("jual_id=?",
        [$jual->id])->sum('sub_total');
        $jual->alamat_kirim_id = AlamatKirim::whereRaw("konsumen_id=? AND is_default=1",
        [auth()->user()->id])->first()->id;
        if($jual==null) return redirect('/konsumen/order')->withErrors(
        ['msg' => 'Cart masih kosong']);
        if(count($jual_details)==0) return redirect('/konsumen/order')->withErrors(
        ['msg' => 'Cart masih kosong']);
        return view('konsumen.jual.checkout', compact('jual','jual_details',
        'alamat_kirims'));
    }
    public function postcheckout(Request $request)
    {
        $alamat_kirim_id = $request->get('alamat_kirim_id', 0);
        $alamat_kirim = AlamatKirim::whereRaw("konsumen_id=? AND id=?",
        [auth()->user()->id, $alamat_kirim_id])->first();
